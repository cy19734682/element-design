<template>
  <div class="em-form">
    <el-form
        ref="elFormRef" :model="valueT" :label-position="labelPosition"
        :label-width="labelWidth" :inline="inline" :size="size" :disabled="disabled"
    >
      <!--解决form只有一个input时enter触发页面刷新的问题-->
      <el-form-item style="display: none"><input type="text" /></el-form-item>
      <el-form-item
          v-for="(item,index) in formData" :key="item.key + index"
      ...
      initDataGroup() {
        for (let root of this.formData) {
          if (root.key) {
            ...
            } else if (root.type === 'checkbox' || (root.type === 'select' && root.multiple) || root.type === 'datetime' ||
                root.type === 'datetimerange' || root.type === 'daterange') {
              if(!isValidVal(this.valueT[root.key])){
